...
parser.add_argument('--data_path',  dest='data_path', type=str, default='F:/dataset/L', help='path of data')
...
parser.add_argument('--widpatchr',  dest='widpatchr', type=int, default=25, help='line template 2')
...
track_params = {}
lprocess = ContactLocate(locate_params,track_params)
...
if __name__ == '__main__':
    ...
    #     fps = (i + 1) / sumTime
    #     saveResult.saveResult(points_r, (i + 1), fps, 'locate+refine.csv')
    ...
    # 3. 增加局部的track
    for i in range(7950, 8300):
        # lprocess.meangrayr = 50
        # lprocess.disr = 3
        image, fImage, _, eqI = ldata.load(i + 1)

        points, points_l, time_process = lprocess.do_multiTrack_update(eqI, fImage, i + 1)
        testShow.plot_locate_result(image, points, 10)

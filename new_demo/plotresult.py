...
class PlotResult(object):
    ...
    def circle_points_on_image(self, image, points, color=(255, 255, 0), s=2):
        # points are N*2 [x,y] numpy
        if len(points.shape) == 1:
            cv2.circle(image, (int(points[0]), int(points[1])), 5, color, s)
        else:
            for i in range(points.shape[0]):
                cv2.circle(image, (int(points[i, 0]), int(points[i, 1])), 5, color, s)
        return image
    ...
    def draw_small_track(self, img, track_output, idx):
        if len(img.shape) == 2:
            img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
        if idx in track_output.index_list:
            index = track_output.index_list.index(idx)
            dict_data = track_output.result_list[index]
            img = self.circle_points_on_image(img, dict_data['points_l'], (0, 0, 255), 1)
            img = self.circle_points_on_image(img, dict_data['points_r'], (0, 0, 255), 1)
            pt1 = (int(dict_data['rect'][0]), int(dict_data['rect'][1]))
            pt2 = (int(pt1[0] + dict_data['rect'][2]), int(pt1[1] + dict_data['rect'][3]))
    ...
    def draw_hash(self, img, track_output, idx, track):
        if idx in track_output.index_list:
            index = track_output.index_list.index(idx)
            dict_data = track_output.result_list[index]
            points = np.vstack([dict_data['points_l'], dict_data['points_r']])
            patch_l_tgt, patch_r_tgt = track.params.patch_l_tgt, track.params.patch_r_tgt
            patch_l, patch_r = get_patch(img, points[0:1, :]), get_patch(img, points[1:, :])
        else:
            zero_p = np.zeros((32, 32), dtype=np.float32)
            return zero_p, zero_p, zero_p, zero_p
        return patch_l_tgt, patch_r_tgt, patch_l, patch_r

    def patch_process(self, patch):
        vis0 = patch.astype(np.float32)
        # 二维Dct变换

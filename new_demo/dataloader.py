...
class DataLoader(object):
    ...
    def load_hornPoints_from_df(self, df, index):
        path = '/VOC2007L/JPEGImages/%06d.jpg' % index
        temp_df = df[df['image_path'].isin([path])]
        temp2 = temp_df[temp_df['left/right'] == 1]
        if len(temp2) > 1:
            temp2 = temp2.head(1)
        leftPoints = np.hstack([temp2.x1.values[0], temp2.y2.values[0]]) if len(temp2) == 1 else np.array([0, 0])
        temp2 = temp_df[temp_df['left/right'] == 2]
        if len(temp2) > 1:
            temp2 = temp2.head(1)
        rightPoints = np.hstack([temp2.x2.values[0], temp2.y2.values[0]])if len(temp2) == 1 else np.array([0, 0])
